...
class App:
    ...
    def _upd_screen(self):
        ...
        floor_x_old, floor_y_old, _ = self._send_ray(x=self.player.x, y=self.player.y,
                                           phi=max_angle - 0*one_step)
        for i in range(H_RES):
            floor_x, floor_y, r = self._send_ray(x=self.player.x, y=self.player.y, 
                            phi=max_angle - i*one_step)
